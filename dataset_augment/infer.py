...
from correction import hist_n_color
...
class SD3Inferencer:
    ...
    def gen_image(
        ...
        for i, prompt in pbar:
            ...
            # Histogram matching and Color Correction
            corrected_image, hist_image, corrected_image_lake, hist_image_lake, corrected_image_sea, hist_image_sea, corrected_image_reinhardt, corrected_image_lake_reinhardt, corrected_image_sea_reinhardt = hist_n_color.color_transfer(image)

            save_path = os.path.join(out_dir, f"pool/{i:06d}_hist_color.png")
            save_path_lake = os.path.join(out_dir, f"lake/{i:06d}_hist_color_lake.png")
            save_path_sea = os.path.join(out_dir, f"sea/{i:06d}_hist_color_sea.png")
            save_path_color = os.path.join(out_dir, f"pool/{i:06d}_color.png")
            save_path_lake_color = os.path.join(out_dir, f"lake/{i:06d}_color_lake.png")
            save_path_sea_color = os.path.join(out_dir, f"sea/{i:06d}_color_sea.png")
            save_path_origin = os.path.join(out_dir, f"{i:06d}_origin.png")
            save_path_hist = os.path.join(out_dir, f"pool/{i:06d}_hist.png")
            save_path_hist_lake = os.path.join(out_dir, f"lake/{i:06d}_hist_lake.png")
            save_path_hist_sea = os.path.join(out_dir, f"sea/{i:06d}_hist_sea.png")
            self.print(f"Saving to to {save_path}")

            corrected_image.save(save_path)
            image.save(save_path_origin)
            hist_image.save(save_path_hist)
            corrected_image_lake.save(save_path_lake)
            hist_image_lake.save(save_path_hist_lake)
            corrected_image_sea.save(save_path_sea)
            hist_image_sea.save(save_path_hist_sea)
            corrected_image_reinhardt.save(save_path_color)
            corrected_image_lake_reinhardt.save(save_path_lake_color)
            corrected_image_sea_reinhardt.save(save_path_sea_color)
...
def main(
    ...
    **kwargs,
    ...
    os.makedirs(os.path.join(out_dir, "pool"), exist_ok=False)
    os.makedirs(os.path.join(out_dir, "lake"), exist_ok=False)
    os.makedirs(os.path.join(out_dir, "sea"), exist_ok=False)
