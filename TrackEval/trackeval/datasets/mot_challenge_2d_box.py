...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['Soda Can'],  # Valid: ['pedestrian']
    ...
    def __init__(self, config=None):
        ...
        self.class_list = self.config['CLASSES_TO_EVAL']  # Use exactly as given
        if not all(self.class_list):
            raise TrackEvalException(
                'Attempted to evaluate an invalid class. Valid classes are: %s'
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            keep_tracker_mask = np.ones(len(tracker_ids), dtype=bool)
            if len(to_remove_tracker) > 0:
                keep_tracker_mask[to_remove_tracker] = False

            if self.do_preproc and self.benchmark != 'MOT15':
                cls_id = self.class_name_to_class_id[cls]
                

            data['tracker_ids'][t] = tracker_ids[keep_tracker_mask]
            data['tracker_dets'][t] = tracker_dets[keep_tracker_mask, :]
            data['tracker_confidences'][t] = tracker_confidences[keep_tracker_mask]
            similarity_scores = similarity_scores[:, keep_tracker_mask]
            ...
            if self.do_preproc and self.benchmark != 'MOT15':
                ...
                gt_to_keep_mask = np.not_equal(gt_zero_marked, 0)
                tracker_to_keep_mask = np.ones(len(tracker_ids), dtype=bool)
